...
seir1 <- function(t, x, parms) {
  ...
  with(as.list(c(parms, x)), {
    ...
    dD  <-      dc *min(Ic,cap)*(1/10)                       + max(((Ic + I1*cc1*gamma + I2*cc2*gamma + I3*cc3*gamma + I4*cc4*gamma)-cap),0) 
...
for(i in 1:n){
  ...
  inits      <- c(S1 = n1 - 1, E1 = 0, I1 = 1, II1 = 0, Ih1 = 0, A1 = 0, R1 = 0, Rh1 = 0, Ic = 0, Rc = 0, D = 0,
...
write.csv(all.scen, './allscenarios_4ageCT_deaths.csv', row.names = F)
...
p1hosp <- ggplot(data = scen1, aes(x = date, y = Ih4, group = scenalpha, color = scenalpha)) +
...
p1icu <- ggplot(data = scen1, aes(x = date, y = Ic, group = scenalpha, color = scenalpha)) +
...
p1d <- ggplot(data = scen1, aes(x = date, y = D, group = scenalpha, color = scenalpha)) +
  geom_line() + labs(x = "Date", y = "Count",
                     title = "Cumulative Deaths, Scenario 1(A-D)",
                     caption = "*May 27th   **May 27th -> June 27th",
                     col = "Scenario") + scale_x_date(date_labels = "%d%b%y", date_breaks = "3 months") +
  guides(color = guide_legend(override.aes = list(size = 10))) +
  scale_color_manual(name = "Scenario",
