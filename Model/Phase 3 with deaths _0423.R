...
seir1 <- function(t, x, parms) {
  ...
  with(as.list(c(parms, x)), {
    ...
    list(der,Id = pID*(I+I2+I3) + Ih+Ic+Ih2+Ih3, Iht =Ih+Ih2+Ih3+Ic)
  })
}
...
for(i in 1:n){
  ...
  parms <- c(beta = scen[i, c('beta')], ## Transmission rate
             ...
             hosp = scen[i,c('hosp')], ##Proportion hospitalized (under 30)
             cc = scen[i,c('cc')],  ## Proportion in ICU (under 30)
             ...
             cap = scen[i,c('cap')] #ICU capacity
             )
  
  dt      <- seq(0, 500, 1)
  
  inits      <- c(S = Cp - n2 - n3 -1, E = 0, I = 1, Ih = 0, A = 0, R = 0, Rh = 0, Ic = 0, Rc = 0, D = 0,
