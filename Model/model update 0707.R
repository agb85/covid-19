...
pop <- read.csv('Data/Scenarios/Data Sets/COpop.csv')
scen <- read.csv('Data/Scenarios/Data Sets/Model params_0707 - model params 0707.csv')
...
seir1 <- function(t, x, parms) {
  ...
  with(as.list(c(parms, x)), {
    ...
    ef1 <- ifelse(t<t2, 0, ifelse(t<t3, mag1, ifelse(t<t4, mag2, ifelse(t<t4a, mag3, ifelse(t<t5, mag3a, ifelse(t<t5a, mag4, ifelse(t<t5b, mag4a, ifelse(t<t6,  mag4b, ifelse( t< t6a, ef1_1, ifelse(t<t6b,  mag5, ifelse (t<t7, mag5a, ifelse(t<t8, ef1_2, ef1_3))))))))))))
    ef2 <- ifelse(t<t7, ef1, ifelse(t<t8,  ef2_2, ef2_3))
    ef3 <- ifelse(t<t7, ef1, ifelse(t<t8,  ef3_2, ef3_3))
    ef4 <- ifelse(t<t7, ef1, ifelse(t<t8,  ef4_2, ef4_3))
...
for(i in 1:n){
  ...
  parms <- c(beta = scen[i, c('beta')], # transmission rate
             ...
             dnh1 = 0.00048, dnh2 = 0.00086, dnh3 = 0.00757, dnh4 = 0.2019,
             ...
             t6a = scen[i,c('t6a')],
             t6b = scen[i,c('t6b')],
             t7 = scen[i,c('t7')], ###Needs to be changed weekly starting July 14th to occur 2 weeks prior to fitting date
             t8 = scen[i,c('t8')],
             ramp = scen[i,c('ramp')],
...
write.csv(all.scen, './Scenarios/Data Sets/allscenarios_0707.csv', row.names = F)
