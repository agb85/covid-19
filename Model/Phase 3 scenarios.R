...
seir1 <- function(t, x, parms) {
  ...
  with(as.list(c(parms, x)), {
    ...
    siI <- ifelse (t < t1, 0, ifelse (t<t4, siI, siI1)) ##Change proportion of symptomatics that self-isolate
    ramp <-ifelse(t < 129, 0, (t-129)*ramp) #For scenario D, E, F : increases proportion of symptomatic case isoaltion over time
    maska <- ifelse(t< t4, 1, maska)
    
    
    dS  <-    - (I+I2+I3)*(beta*lambda*S*(1-(siI+ramp))*(1-ef))/N - (beta*S*maska*(A+A2+A3)*(1-ef))/N 
    dE  <-    - E/alpha   + (I+I2+I3)*(beta*lambda*S*(1-(siI+ramp))*(1-ef))/N + (beta*S*maska*(A+A2+A3)*(1-ef))/N 
    dI  <- (E*pS)/alpha - I*(gam) 
    ...
    dS2  <-    - (I+I2+I3)*(beta*lambda*S2*(1-siI)*(1-ef))/N - (beta*S2*maska*(A+A2+A3)*(1-ef))/N 
    dE2  <-    - E2/alpha   + (I+I2+I3)*(beta*lambda*S2*(1-siI)*(1-ef))/N + (beta*S2*maska*(A+A2+A3)*(1-ef))/N 
    ...
    dS3  <-    - (I+I2+I3)*(beta*lambda*S3*(1-siI)*(1-ef3))/N - (beta*S3*maska*(A+A2+A3)*(1-ef3))/N 
    dE3  <-    - E3/alpha   + (I+I2+I3)*(beta*lambda*S3*(1-siI)*(1-ef3))/N + (beta*S3*maska*(A+A2+A3)*(1-ef3))/N 
...
scen <- read.csv('data/phase 3.csv')
...
write.csv(all.scen, 'data/allscenarios_phase3.csv', row.names = F)
